...
- **回归套件开始检查维护文档**：自测套件现在会验证维护文档里的引用是否可解析、路线图是否保持四个必需段（文档不存在时整组跳过——如 CI 全新检出），于是 CI 结构上看不见的漂移能在本地被抓住。场景数相应变为 258。
