...
- **委托请求在请求时刻按任务声明的执行方式做归属校验**：标记为 pending 的任务，仅当工作流已位于其声明所对应的节点——并行任务对应委托节点、串行任务对应执行节点——时才会被接受。校验发生在写入任何交接证据之前，因此拒绝时状态文件逐字节不变、不落任何记录；提示给出恢复路径：先 `workflow-state next` 校正，再 `entry <目标节点>`。节点正确但尚未 entry 只告警；已完成任务补录与显式 `--write-files` 请求不受门禁约束；协议没有对应启用节点时跳过校验；旧 change 保持此前的告警后继续行为。
- **受控修复回路按 change 计数，第 4 次暂停等待决策**：从 review 或 verify 节点带着待修复工作归位回执行节点时，该 change 的专用计数加一；前三次在审计行打印当前轮次，新 change 的第 4 次归位被阻断且不推进工作流，并给出「继续修 / 停止」指引。继续需要用户显式授权并记入交接证据；旧 change 告警后照常放行。verify 命令失败使用另一个按 change 隔离的计数，verify 成功不清零修复轮次；不属于这种受控归位的路径——任务集已完成但未闭合、仅回程分类、verify 命令失败——均不计入。
- **任务集签名携带产生它的算法版本**：记录的签名值带版本前缀，应用出口事件同时记录算法版本，因此旧 state 写下的裸摘要按最初版本的语义读取。回程分类、受控归位与任务集比对只在同一算法版本内比较，引擎升级后不会把正常多趟收尾误读成未闭合的修复回路，也不会误拦未变化的任务集；算法版本本身的更换必须在冻结的回归锚上显式决策，而不是静默重新分类既有状态。
- **声明零提交的交接在记录结果时重新核验**：结果若声明没有提交，必须在记录结果时重新证明其声明输出为空或确实被忽略规则覆盖。若期间忽略规则变化、声明路径变为 tracked，或任一已存在的祖先段是符号链接/junction，则已记录的零提交标记被撤销：新 change 上结果被拒绝（旧 change 告警），不再豁免缺失的提交哈希；判定前先对项目根做 realpath 归一。无法证明属于零提交类的路径仍走完整提交子集校验。该检查是判定时刻的路径安全证明——未被提交引用的提交、判定后 HEAD 移动、校验与记录之间的竞态，以及 worktree 隔离下不可见的提交，仍是已登记的残留风险，机制不声称已闭合。
- **任务摘要按完整模板骨架机检**：SUMMARY 模板中的每个段现在都必须存在——条件段可显式写 N/A——已出现的段必须符合模板顺序，强制的自检方法段须位于越界检查之后（文末或紧随其后）。新 change 缺段即阻断并给出补 N/A 的恢复指引；旧 change 保持渐进告警；模板缺失时回退内置骨架。
- **安装器的消息改用工具自己的名字**：输出前缀取自脚本自身的文件名，于是用户敲 `fcomet`、每一步却都在一个他从未用过的名字下汇报——而同一份输出里的用法行已经写着 `fcomet init`，一条命令在同一屏里有两个自称。前缀现为 `flow-comet`，与工具在任何调用形态下的名字一致（包内命令、其别名、仓库克隆里的脚本）。
- **注入到项目的编排说明不再带过时用词与非入口项**：首行曾称其为 "bundle" 与 "portable control plane"——本仓在脱离前身目录结构时早已弃用的词；脚本清单里还列着一个只有两行的兼容别名壳，它不是任何流程的入口。首行现直接说明该文件是什么，别名壳不再被列出。该文本**每个项目都会自动加载**，现由一条检查守着，不让这两类内容回来。
- **回归套件开始检查维护文档**：自测套件现在会验证维护文档里的引用是否可解析、路线图是否保持四个必需段（文档不存在时整组跳过——如 CI 全新检出），于是 CI 结构上看不见的漂移能在本地被抓住。回归套件现运行 258 场景，系统测试集 78 项。
- **文档与已发布的行为对齐**：安装指南不再指向已退役的状态文件与命令；npm 通道按已发布事实描述；发布清单补上 npm publish 与 GitHub Release 两步；Codex 一节改为描述串行交付（不再是手工 worktree 委托）；force-push 规则写明适用范围；Comet 徽章旁加了一句界限说明（两个项目已不再共享文件）。
- **修复批次现在真实经过执行节点生命周期**：review / verify 报出缺陷后，后续修复工作被路由回执行节点，归位真实写入工作流状态，执行节点的四类出口因此真实判定——全任务完成、逐任务摘要齐备、自检证据已声明、任务集签名未变。修复通过后，工作流回到报出缺陷的节点，先跑该节点自己的出口门禁再继续推进，不再因报告已存在而跳过。
- **修复回程的审计行恢复判别力**：修复回路重新进入执行节点家族、随后把控制权交回报出缺陷的节点时，打出的行按成因分类——真实修复批次保留专用的修复标签，正常多趟收尾改打中性的回程行，不再借用该标签；已记录状态不足以区分两者时，改打中性行并标注未分类，而不做猜测；把源节点留在原位以便其自身出口运行的那一行始终中性。路由、已完成节点与状态写入均不变，只有审计文案不同。
- **声明输出为空或全部被忽略的交付可零提交闭环**：委托任务的声明输出为空、或每一项都被仓库的忽略规则覆盖时，现可合法地以零提交完成交接——结果可以不写提交哈希，也不再产生交付错误。无法证明属于该类的情形仍执行完整的提交子集校验；声称零提交却无此证明的结果在新 change 上被拒绝（旧 change 告警），因此这一合法形态不会绕过该校验。
- **安装副本侧无法适用的检查改为显式报告**：只有权威源树才能执行的 loader 版本戳检查，在套件从安装副本运行时现会输出「不适用」，不再静默返回并被计为通过；运行器对两侧都断言机器可读的结果标记，标记缺失或不匹配即判失败，无法蒙混过关。
...
- **修复批次不能再以「直接退出报出节点」收场**：旧的路径——留在报出缺陷的节点上、把它直接关闭——现被拒绝并给出恢复指引。该路径让修复工作完全绕过执行节点生命周期，四类出口一次都不跑且零报错，而报出节点自己的门禁也可能因报告已存在被跳过；唯一被接受的路径是经过执行节点、再从报出节点的出口门禁回来。
...
- **标题带合法闭合标记的上下文文档不再阻断流程**：`## 项目概要 ##` 这类 ATX 闭合写法会把结尾的 `##` 一并存进段名，段名比对因此落空，结构校验把实际存在的段报成缺段——现剥离闭合标记（仍要求它前面有空白，故以 `#` 结尾的合法段名不受影响）。
- **安装器不再截断读不到的 `.gitignore`**：此前所有读取错误都被当成「文件不存在」，于是在「读被拒、写允许」的访问故障下，用户既有 `.gitignore` 会被改写到只剩受管条目。现只有确实不存在的文件才算首次纳管，访问类故障改为中止安装——这条判据安装器此前已用于迁移探测，现由两处共用。

### 变更

- **受保护路径的判据改为单一实现**：写入守卫曾自带一份逐字节相同的路径包含性、受保护路径检查与受保护文件读取实现，改其中一份永远到不了另一份——现由共享模块导出、守卫引用。
- **过时表述订正**：产物根错误消息不再声称「只有状态机推导不支持」（guard 同样拒绝这些取值，相邻注释一并订正）；dsh 平台参考文档把 npm 包记为已发布、AGENTS.md 注入记为已认证；安全策略列出安装器的第三方依赖，并写明该依赖自身漏洞的报告边界在上游。
- **CHANGELOG 自身的路径引用补全**：权威版本标识改述为 `.flow-comet/skills/flow-comet/INSTALLED_VERSION`——该路径在仓库根可解析。
- **三处技能描述与它们所描述的机制对齐**：review 参考写明缺失处置标记的两种结果（新 change 缺失即阻断、旧 change 告警渐进），evolve 说明时间戳过期只提示显式调用、不自行触发，decision-points 不再把终止性停止条件划为用户决策。
- **回归套件现运行 243 场景**；系统测试集保持 75 项不变。
...
- **npm 分发通道**：flow-comet 现可作为 npm 包安装，并把安装器以两个命令名（`fcomet` 与 `flow-comet`）暴露出来——全局安装后执行 `fcomet init` 即在当前目录完成项目安装；`--target` 成为可选项，安装器同时接受可选的前导 `init` 词元，因此 `fcomet init` 与裸 `fcomet` 等价。发布内容以 `files` 白名单声明（技能树、编排规则、安装器与 dsh 桥接脚本），私有产物与「不分发」产物由结构排除，而非依赖黑名单过滤。本批次做到「可发布」，随后的 `1.5.0-rc.3` 发布即把它发布出去——registry 上的首个版本。
...
- **包清单版本纳入发布版本面**：发布一致性检查现对账九处而非八处——npm 包的 `version` 与 README 徽章、本 CHANGELOG、[docs/VERSIONS-zh.md](docs/VERSIONS-zh.md) 及权威源 `.flow-comet/skills/flow-comet/INSTALLED_VERSION` 同值比对，带着过期版本号的包会在发布检查处失败，而不会流到 registry。
...
- **本仓遗留的 Comet 配置文件**：`.comet/config.yaml` 使用 Comet 的项目 schema，但从未在本仓生效——本仓没有 classic change 目录，代码库中也没有任何读取方。它位于本仓的文件树且由本仓提交，故移除是本仓可自行决定之事；本仓现已不含 `.comet` 目录。
...
- **worktree 隔离的子代理不再被误判为协调者**：项目根锚定链（上一版本引入）经宿主注入的变量解析到主仓库——而主仓库位于隔离工作区之上——导致 `.claude/worktrees/**` 内的写入被按协调者白名单判定并拦截，尽管「worktree 子代理写源码」正是设计本意。守卫现放行该隔离前缀（含 `../` 穿越防护）；机制说明中此前表述为「项目 hook 对子代理不生效」的记载，现改为陈述其真正成立的条件。
- **状态路径失效时给出自解释的报错**：当协议的 state path 指向不存在的文件时，守卫现报告解析出的路径、探测另一运行时命名空间下是否存在状态文件（提示项目可能尚未迁移），并指出应当修正的字段——而非只抛一句干巴巴的「state does not exist」。仍保持 fail-closed，且绝不回退。
- **运行时状态路径收敛为单一来源**：该字面量此前重复出现在四个脚本中，改动一处不会让其他处报警；现统一由一个导出常量派生，写入守卫因此新增的模块导入实测约 1.7 毫秒/次。
- **Claude Code hook 命令全平台统一为 POSIX 形态**：安装器此前在 Windows 上生成 cmd 风格的 `%CLAUDE_PROJECT_DIR%` 引用与反斜杠路径，而宿主以 bash 语义执行 hook 命令——变量不展开、反斜杠被当转义吞掉，守卫脚本在每次工具调用时加载失败（宿主将崩溃的 hook 降级为非阻塞错误，写入拦截随之失效）；现全平台生成花括号 POSIX 形态，回归套件验证生成命令在宿主语义下真实可执行。
- **迁移与安装器据外部审查加固**：修正四处行为。① worktree 隔离区放行现同时覆盖**由 Bash 命令判定的写入**，不再只覆盖由文件路径判定的那一路——此前同一次写入经一条路径放行、经另一条被拦。② 只有当路径**确实不存在**时才按不存在处理；权限失败一类的访问错误现会浮出并中止，而不再被静默读作「无旧状态」——后者可能在报告迁移完成的同时把项目的状态留在原地。③ 以 UTF-8 BOM 开头的状态文件在校验时被接受（与运行时读取一致），而备份与迁移后的文件仍保留原始字节。④ `--purge` 的确认校验提前到任何写入之前——最终被拒绝的命令不再先行修改目标。
- **契约检查器纳入共享模块**：`protocol-utils`、`state-schema`、`task-parsing` 与 `route-node` 现属其必需清单——副本若缺其中之一，所有守卫脚本都会在导入时报错，而检查器此前仍报告成功。
- **worktree 委托说明不再记载绕过手段**：此前推荐用 File API 直接写入以规避写入守卫的做法，现标记为已撤回的规避手段并禁止使用；在不支持子代理隔离的平台上，委托环节现要求串行执行。
- **任务属性与命令行校验据外部审查修正**：任务解析现接受单引号属性值（此前这类任务被静默读作非并行，导致路由偏移并跳过并行写重叠检查）；契约检查器在构造正则前先转义字段名（此前字段含方括号会抛错，形如 `item[0]` 的字段会误匹配无关文本）；`--project` 现优先于工作目录，且缺值时明确报错（此前显式给出的根被静默忽略）；上下文初始化检查改为精确匹配标题而非子串包含（此前标题变体——甚至正文里的一句提及——都能让校验通过并记录扫描时间）。
- **隔离区放行现校验物理包含性**：本批次早先加入的放行只比对归一化后的路径，因此 worktree 内**指向其外部**的符号链接或 junction 仍会被放行——而两条判定路径都在任何后续包含性检查**之前**返回，没有任何环节能兜住。现改为逐分量解析，要求目标的**真实位置**仍落在预期 worktree 之内；目标尚不存在时，按最近的已存在祖先推导落点。解析失败一律拒绝而非放行。该校验仅在词法前缀命中时执行，普通会话零开销。
- **上下文校验忽略围栏代码内的标题**：段位检查此前会匹配出现在任何位置的 `##` 行，因此缺必需段的文件，只要代码示例里恰好含有那个段名，照样能通过——并像文件完整时一样记录扫描时间。现先跟踪并排除围栏代码块，再收集段集合。
...
- **验证出口命令执行兼容 dsh headless 受限沙箱形态**：管道式执行被拒（EPERM，实测见于 dsh headless 一次性 runner 会话）时，guard 以继承 stdio 重试同一命令——真实执行与退出码判定不变，降级捕获以 VERIFY-DEGRADED 行标记；非 EPERM 失败不降级。([#67](https://github.com/baobaolaodie/flow-comet/pull/67))
- **零提交任务不得携带 tracked 提交**：声明 noCommit 的结果若提交包含 tracked 文件，新 change 予以拒绝（旧 change 告警），封堵提交子集边界的逃逸口；**入口文档在新 change 强制 Change ID 首部**（此前仅告警）。回归套件扩展至 171 场景。([#67](https://github.com/baobaolaodie/flow-comet/pull/67))
