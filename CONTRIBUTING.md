...
- **Repo**: clone, run `npm install`, then verify the regression baseline runs:
  `node .flow-comet/skills/flow-comet/scripts/guard-self-test.mjs` → `ALL 231 SCENARIOS PASSED` (two-tier baseline; also run `system-test.mjs` → `ALL SYSTEM TESTS PASSED`, 74 items)
...
```bash
node .flow-comet/skills/flow-comet/scripts/guard-self-test.mjs   # → ALL 231 SCENARIOS PASSED
```
...
- **Regression baseline**: `node .flow-comet/skills/flow-comet/scripts/guard-self-test.mjs` → `ALL 231 SCENARIOS PASSED` (two-tier baseline; also run `system-test.mjs` → `ALL SYSTEM TESTS PASSED`, 74 items) (mandatory after every change)
...
- Verification: regression (231 scenarios) / installed-copy checks
