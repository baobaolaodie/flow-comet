...
- **Verify-exit command execution compatible with restricted sandbox sessions**: when a piped spawn is denied (EPERM, e.g. dsh restricted sessions), the guard retries the same command with inherited stdio — real execution and exit-code judgment unchanged, a VERIFY-DEGRADED line marks the degraded capture; non-EPERM failures never degrade. Regression suite expanded to 169 scenarios. ([#67](https://github.com/baobaolaodie/flow-comet/pull/67))
