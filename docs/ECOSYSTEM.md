...
| 8 | **Entry Skill dual-zone structure** (deterministic Auto zone: route table/Skill Bindings/Guardrails/Recovery + Authored zone: Decision Core with four mandatory sections) | SKILL.md isomorphic; Decision Core has the same four sections (auto node detection / decision classification / stop conditions / Red Flags) |
...
| 10 | **eval.yaml manifest** (comet.eval/v1alpha1 + qualityGates + routeConformance) | `comet/eval.yaml` isomorphic; `checks.yaml` (state_equals), `skill.yaml`, `guardrails.yaml` present |
