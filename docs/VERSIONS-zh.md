...
| 项 | 说明 |
|----|------|
| **当前版本** | v1.4.0（记录于 [CHANGELOG-zh.md](../CHANGELOG-zh.md)、git tag、README 徽章与权威源 `skills/flow-comet/INSTALLED_VERSION`；git tag 在发布步骤创建；v1.0.0 = 首个稳定版：8 节点工作流 + 三层防线 + guard 校验体系） |
...
- 回归基线（两级）：`guard-self-test.mjs` 134 场景 + `system-test.mjs` 50 项全绿（每次改动后必须）
