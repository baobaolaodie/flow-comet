...
Two command names ship in the package and both point at the same installer: `fcomet` (primary) and `flow-comet` (alias). The leading `init` token is optional whenever another argument expresses the intent (`fcomet --target <dir>` is equivalent to `fcomet init --target <dir>`), and `--target <dir>` is optional as well (default: the current working directory), so running the command from inside the project is enough. Called with no arguments at all, the command prints its usage and exits non-zero rather than installing. `--platform <claude-code|codex|dsh|claude-code,dsh|all>` selects the platform without a prompt, `--purge --yes` resets an installation (delete-and-rebuild, **not uninstall**), and `--version` (or `-v`) prints the version of the installed CLI.
...
**Version marker**: the marker written by an npm install (`<target>/.claude/skills/flow-comet/INSTALLED_VERSION` for Claude Code, `.agents/skills/…` for Codex, `.dsh/skills/…` for dsh) is the release version shipped inside the package. The `<release>-<n>-g<hash>` development marker is derived from `git describe` in the source repository, so it only appears for installs run from a repository clone that has git and tags ([Option B](#option-b--prepare-env-installer-repository-clone)) — an npm install has no repository history to derive it from.
...
### Verifying installation (Claude Code; no side effects, no change created)
...
- **Git proxy operations (measured)**: git is restricted inside the codex sandbox (init branch creation fails) — init branch failure degrading to file-only mode is expected; commits from delegated subagents are made by the coordinator outside the sandbox, and the Return Contract's commitHash remains verifiable.
