...
| dsh skill not visible in a session | dsh-flow-comet not installed, not activated for this project, or dsh below `0.1.0-rc.6` | run `dsh plugin --profile <name> add dsh-flow-comet`; confirm the session project root contains `.comet/` or `.specs/` traces (project-level default); upgrade dsh to `0.1.0-rc.6`+ |
| dsh interception not blocking writes | plugin not loaded, project not activated, or `tools/pre-execute` signature mismatch | confirm the plugin is installed and the session project is activated; verify dsh is `0.1.0-rc.6`+; reinstall or re-activate the plugin |
| dsh uninstall residue (AGENTS.md block / protocol copy remains) | `dsh plugin remove` alone does not run the plugin cleanup channel | run `node <plugin-path>/scripts/cleanup.mjs` first (strips the AGENTS.md managed block + removes `<project>/reference/.flow-comet-workflow-protocol.json`), then `dsh plugin --profile <name> remove dsh-flow-comet` |
| dsh audit log keeps growing | `$DSH_HOME/flow-comet-audit.jsonl` is append-only and cleanup intentionally does not delete it | expected; delete the file manually when the audit trail is no longer needed |
