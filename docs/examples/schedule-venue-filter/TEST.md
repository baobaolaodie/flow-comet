...
- **前置**: 登录 seed 管理员（13900000000）；存在启用场地 `v1="示例体育馆"`（归属赛事 A）与 `v2="示例文化宫"`（归属赛事 B）
- **步骤**: 1. 进入管理端"赛事列表"页；2. 场地下拉选择"示例体育馆"；3. 观察列表行"场地"列；4. 切回"全部场地"
- **期望**: 步骤 2 后仅剩赛事 A，且其行"场地"列为"示例体育馆"；步骤 4 恢复全部赛事，无场地的赛事显示"—"
...
- **步骤**: 1. 场地下拉选"示例体育馆"；2. 状态下拉选"进行中"
- **期望**: 仅返回"包含示例体育馆 且 status=5"的赛事 A
...
- Unicode / 特殊字符：场地名含中文（"示例体育馆"）join 正常，前端 ellipsis 截断长名
