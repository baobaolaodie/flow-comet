...
// 产物根解析：'project'/缺省 → runRoot；'specs-root' → runRoot/.specs。
// classic/native 等 pathBase 依赖项目配置文件，已随 Comet 感知层剥离——不再读取任何项目配置，
// 遇到即显式报错（fail-fast，与 route-node.mjs 的状态机推导侧同语义），不静默兜底。
async function workflowArtifactRoot(pathBase) {
  if (pathBase === undefined || pathBase === 'project') {
    await inspectWorkflowProtectedPath(runRoot, runRoot, 'workflow path base', 'directory');
    return runRoot;
  }
  if (pathBase === 'specs-root') {
    return path.join(runRoot, '.specs');
  }
  throw new Error('产物根 pathBase "' + pathBase + '" 不再支持——请改用 specs-root/project + 完整路径（如 project + openspec/changes/xxx.md）');
}
      ...
      const exists = (
        await Promise.all(
          (artifact.paths ?? []).map(async (artifactPath) => {
            ...
            return pathPatternExists(await workflowArtifactRoot(artifact.pathBase), resolvedPath);
          }),
        )
      ).some(Boolean);
  ...
  if (node.id === 'execute' || node.id === 'subagent-execute') {
    ...
    for (const f of summaryFiles) {
      try {
        ...
        // 段名允许带括号后缀（如 "## 自检方法（声明 brooks-review 或 builtin-quickcheck）"——括号为说明非段名部分，
        // 执行者常按模板标题原样书写导致旧正则不匹配落入"旧格式 WARN"；括号后缀与段名等价）
        ...
        const methodMatch = content.match(/##\s*自检方法(?:[（(][^）)\n]*[）)])?\s*\n([\s\S]*)/i);
