...
- **与引擎回归的关系**：`guard-self-test.mjs` 是引擎脚本的单元/场景级回归（fixture 构造为主，235 场景）；本系统测试集是**系统级**（74 项：A1~A20 / B1~B9 / C1~C3 / D1~D3 / E1~E2 / F1~F4 / G1~G3 / H1~H4 / I1~I4 / J1~J2 / K1~K17 / L1~L3）——每个测试项走真实命令链路，验证机制在真实调用序列下生效（行为锚定归场景回归，集成正确性归系统测试）。
