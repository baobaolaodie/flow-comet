#!/usr/bin/env node
// C1 · flow-comet 引擎自测套件（135 场景：节点门禁 entry/exit 校验正反例与 WARN 渐进、自定义协议加载路由与防线、TASK 签名与 next 推进、handoff Return Contract 与时间序、init 状态机与 hook 写白名单、CONTEXT 自动初始化检测、completedChecks 真实性声明机制（skill-load/record/exit 校验 + 交叉自洽 + 旧兼容）、init 参数误用防护、执行遗漏防护、严格模式、场景数一致性自检）
//
...
// 全过 → exit 0，输出 ALL 135 SCENARIOS PASSED；失败 → exit 1，列出场景名+实际输出+exit code
//
