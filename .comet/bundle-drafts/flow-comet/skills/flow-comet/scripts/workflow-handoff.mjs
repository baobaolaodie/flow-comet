  ...
  if (action === 'result') {
    ...
    try { parsed = JSON.parse(raw); } catch {
      ...
      if (looksLikeObjectLiteral(raw)) {
        console.error('handoff result 解析失败: payload 形似对象但 JSON.parse 失败——请用 --json-file <path> 传参（Return Contract 应传合法 JSON,Windows 传参会剥离内嵌引号）; 未写 handoffResult。原始 payload 前 200 字符: ' + String(raw).slice(0, 200));
        process.exit(1);
      }
    }
    state.evidence = state.evidence || {};
