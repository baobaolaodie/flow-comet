...
async function main() {
  ...
  if (command === 'init') {
    ...
    if (changeName.startsWith('--')) {
      throw new Error('init: ' + changeName + ' looks like a flag, not a change name. Usage: workflow-state.mjs init <change-id> [--branch-prefix <prefix>] [--init-context|--init-skip]');
    }
