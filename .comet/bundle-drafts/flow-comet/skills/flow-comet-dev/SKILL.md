...
**覆盖声明（上游 vendored 只读）**：vendored 上游提示文件（`flow-kit/prompts/**`、`flow-kit/templates/**`）保持只读基准，不作修改——其中与本仓不一致的旧版原子提交格式示例不采用，提交格式与本仓约束以本节为准；已登记向上游提改进待办（REQUIREMENT v2），收口前由本声明压制差异。
