...
**约束本体**：标记为并行的任务必须构成单个连续块，且位于整个波次序列的首或尾；序列中段禁止把并行块拆散成混排形态。

合法形态示例（P = 并行波次，S = 串行任务）：`P+S`（并→串）、`S+P`（串→并）、全并、全串。非法形态示例：`S→P→S`（串→并→串）、`P→S→P`（并→串→并）。

**警示**：违规会在规划出口被拦，返工成本高，落笔时即守。

**理由**：当前引擎委托节点为单趟访问设计——并行委托只进一次，混排序列无法被多趟消化。
